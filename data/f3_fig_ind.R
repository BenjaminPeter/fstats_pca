...
pcs2 = pcs %>% left_join(f3, by=c(pop="pop1")) %>% 
    mutate(col=as.factor(cut(est, c(-1,-3e-4,1e-10,2), labels=F) ))
P1 = ggplot() + 
...
f3pc = of3_from_pc_matrix(pcmat, idx, idy) %>% 
    ...
    left_join(f3, .) %>% 
    mutate(PC1_20=rowSums(across(PC1:PC20)))
    mutate(PC1_2=rowSums(across(PC1:PC2)), PC1_10=rowSums(across(PC1:PC10)))
...
P6 = f3pc %>% pivot_longer(PC1:PC20) %>% 
    ...
    geom_boxplot() + 
    ylab("f3(Mbu.; Moz., X)") + 
    scale_x_continuous(breaks=1:20) + BTHEME
}
...
ggsave( "f3_iwesteurasia1.png", P1, width=3.5, height=3, scale=1.5)
ggsave( "f3_iwesteurasia2.png", P2, width=1.75, height=1, scale=1.5)
ggsave( "f3_iwesteurasia3.png", P5, width=1.75, height=1, scale=1.5)
#ggsave( "f3_iworld1.png", P3, width=3.5, height=3, scale=1.5)
#ggsave( "f3_iworld2.png", P4, width=1.75, height=1, scale=1.5)
#ggsave( "f3_iworld3.png", P6, width=1.75, height=1, scale=1.5)
