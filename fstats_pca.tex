...
\subsection{Introduction to MDS}


	
\section{Relationship of PCA, $F_2$ and Outgroup-$F_3$}
	Let us assume we have some genotype data $\MX$, which contains allele-frequency data from $n$ populations on the rows, and $k$ loci on columns. Each population may be represented by a single haploid, pseudo-haploid or diploid individual. Thus, a PCA where each row represents a single individual \cite{patterson2006} is included in this framework. 
	...
	\begin{figure}
		...
		\caption{Basic Idea of PCA from 2D to 1D representation}
