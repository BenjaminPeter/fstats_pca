...
\begin{figure}[!ht]
	...
	\caption{\textbf{Admixture-$F_3$-statistic and Dimensionality reduction.} A: $F_3(X_x; X_1, X_2)$ in two dimensions (full data). Points like $X_x$ inside the circle (red) have $F_3 <0$, but points outside ($X_3, X_4$, blue) have positive $F_3$-statistics. B: Projection on one dimension. The full rejection region (light gray bar) is larger than that predicted from the distance between $X_1$ and $X_2$ (dark grey bar). Points with negative $F_3$ are guaranteed to lie in the light gray (but not dark gray). $X_3$ also projects onto the gray bar, even though its $F_3$ is positive. Points such as $X_4$ that project outside the rejection region have $F_3>0$.}
	\label{fig:proj}
\end{figure}
