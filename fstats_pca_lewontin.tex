...
The two data sets I analyzed here suggest that two PCs (for the World data set) respectively three PCs (for the Western Eurasian data) already provide a very good approximation for $F_4$-statistics (Figures \ref{fig:weu}E, \ref{fig:world}E), reflecting the observation that frequently the first few PCs provide a good approximation of population structure. Thus, we may expect that whenever the first few PCs provide a good approximation of population structure, this results in well-approximated $F_4$-statistics. On the other hand, for admixture $F_3$-statistics, more PCs are needed (Figure \ref{fig:weu}B), likely because these statistics do contain a term measuring local variation in the putative admixed population \citep{peter2016}, which is typically often incorporated on higher PCs. Thus, I would also expect other $F$-statistics including large contribution from local variation such as  $F_4$-statistics between closely related populations to require substantially more PCs.
