...
One difficulty in interpreting my results is that they only hold when all PCs are considered, and thus are approximations when considering only the first few PCs. However, for the two  data sets I analyzed here, two PCs (for the World data set) respectively three PCs (for the Western Eurasian data) already provide a very good approximation for $F_4$-statistics (Figures \ref{fig:weu}E, \ref{fig:world}E). Thus, we may expect that whenever the first few PCs provide a good approximation of population structure, this results in well-approximated $F_4$-statistics. On the other hand, for admixture $F_3$-statistics, more PCs are needed (Figure \ref{fig:weu}BC), likely because these statistics do contain a term measuring local variation in the putative admixed population \citep{peter2016}, which is typically often incorporated on higher PCs. Thus, I would also expect individual-based analyses and $F_4$-statistics between closely related populations to require substantially more PCs as there local differentiation, which is typically reflected on higher PCs, becomes important.
...
The liability of PCA to missing data is a well-studied problem and a number of algorithms for imputing missing data have been proposed \citep[e.g.][]{hastie2015, meisner2018, meisner2021}. Calculating $F$-statistics first is yet another way to impute data for PCA that works well if missingness is low \citep{meisner2021}. In contrast, missing data in $F$-statistics is handled by estimating a standard error using resampling across the genome \citep{patterson2012}, which does not distinguish between biological and sampling variation. These strategies are distinct, but not unique to the relative approaches and a PCA-like decomposition from $F$-statistics is commonly applied using MDS \citep[e.g.][]{fu2016}. One alternative suggested by the theory developed here could be to first perform a PCA to impute missing data, and then calculate $F$-statistics using this imputed data set. Using this approach, the accuracy of $F$-statistics including low-coverage samples would be greatly increased, as they can ``borrow'' information from closely related samples.
...
Thus, a more abstract way of interpreting $F$-statistics is that they reflect the relative position of population to each other in PCA-space. The classical model of admixture as a result of contact between long-separated and isolated populations is just one of potentially many demographic models that might result in this data pattern.
...
In this vein, admixture tests based on $F$-statistics evaluate if drift in different parts of the tree are orthogonal to each other. This orthogonality is expected to arise whenever drift in different populations is independent, which happens, for example, if we consider two isolated populations, or if we consider the changes in allele frequency in distinct time intervals. 

In the context of tree-like population models, this orthogonality arises from drift on non-overlapping branches.  
...
Interpretations of the outgroup-$F_3$-statistics and $F_4$-statistics in a PCA-framework rely heavily on the geometric concepts of projections and orthogonality: In a tree or admixture-graph framework, we can loosely interpret $F_4(A, B; C, D)$ as a projection of one the path $A-B$ onto $C-D$; only edges shared between the paths will contribute non-zero amounts to this statistic. This interpretation holds when we replace the populations by points in PCA-space, which enables us to study population models beyond discrete graphs. Thus, the geometric framework enables us to expand the application of $F$-statistics beyond current uses, allows us to better understand the meaning of these statistics and helps us to avoid overinterpretations particularly in cases when populations structure is continuous.
